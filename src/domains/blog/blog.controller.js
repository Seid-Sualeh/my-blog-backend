...
class BlogController {
  ...
  async toggleLike(req, res, next) {
    try {
      const { id } = req.params;
      const result = await blogService.toggleLike(id, req.writerId);
  ...
  // Toggle favorite on blog
  async toggleFavorite(req, res, next) {
    try {
      const { id } = req.params;
      const result = await blogService.toggleFavorite(id, req.writerId);
  ...
  async getFavoriteBlogs(req, res, next) {
    try {
      const options = {
      ...
      const result = await blogService.getFavoriteBlogs(req.writerId, options);
  ...
  async getLikedBlogs(req, res, next) {
    try {
      const options = {
        page: parseInt(req.query.page) || 1,
        limit: parseInt(req.query.limit) || 10,
      ...
      const result = await blogService.getLikedBlogs(req.writerId, options);
